# 🤖 Consilium AI - Мультиагентная система для создания конспектов
...
*   **Модульная архитектура:** Код разделен на логические модули (как правило, до ~250 строк) для удобства поддержки и развития.
...
*   **Валидация промптов:** Агент-промптер анализирует и улучшает пользовательские запросы перед генерацией.
...
Проект организован в модульную структуру (большинство модулей компактные, до ~250 строк):
...
├── prompter.py            # Узел промптера для валидации запросов
...
graph TD
    ...
    Prompter -->|Промпт валиден| Generator
    Prompter -->|Нужны уточнения| AskUser(Прервать и спросить пользователя)
...
*   **config.py** - Централизованное хранение всех констант, путей к файлам, настроек LLM и параметров агентов.

*   **database.py / db_schema.py** - Работа с базой данных SQLite: создание чатов, сохранение и загрузка сообщений.

*   **agent_logging.py** - Логирование всех ответов агентов в JSONL формате для последующего анализа.

*   **llm_setup.py** - Инициализация LLM (Deepseek), загрузка промптов из файлов и создание шаблонов.

*   **decision.py** - Определение структуры состояния графа (AgentState) и функция принятия решений о следующем шаге в основном цикле.

*   **prompter.py** - Узел промптера для анализа и улучшения пользовательских запросов перед началом генерации.
...
2.  **Создайте файл `.env`:**
    ...
    # Опционально: отдельные ключи для генераторов (имеют приоритет над общим ключом)
    # Поддерживаются оба формата имен:
...
*   **Мультиагентный подход:** 6 специализированных агентов (промптер, 3 генератора, критик, редактор) работают совместно под управлением LangGraph.
...
*   **Поддержка нескольких API ключей:** Возможность использовать разные API ключи для каждого генератора для распределения нагрузки.
...
- **Агенты** (`generator.py`, `critic_editor.py`, `prompter.py`) - специализированные LLM-агенты
