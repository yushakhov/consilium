"""
Модуль с компонентами пользовательского интерфейса.

Содержит функции для отображения отдельных частей UI:
- render_sidebar: Боковая панель с историей чатов
- render_messages: Отображение сообщений в чате
- load_chat: Загрузка чата из БД

Вынесен в отдельный модуль для соблюдения ограничения на количество строк.
"""

import streamlit as st
from database import get_chats, get_messages


def load_chat(chat_id: int):
    """
    Загружает чат из базы данных и обновляет состояние сессии.
    
    Args:
        chat_id: ID чата для загрузки
    
    Side Effects:
        - Обновляет st.session_state.chat_id
        - Обновляет st.session_state.messages
        - Сбрасывает состояние графа
        - Сбрасывает флаг ожидания ответа пользователя
        - Перезапускает приложение (st.rerun)
    """
    st.session_state.chat_id = chat_id
    st.session_state.messages = get_messages(chat_id)
    st.session_state.current_graph_state = None
    st.session_state.awaiting_user_response = False
    st.rerun()


def render_sidebar():
    """
    Отображает боковую панель с управлением чатами.
    
    Содержит:
    - Кнопку создания нового конспекта
    - Загрузчик файлов (.txt, .md)
    - Список истории чатов
    
    Returns:
        str: Содержимое загруженного файла (если есть)
    """
    with st.sidebar:
        st.title("Consilium")
        st.markdown("---")
        
        # Кнопка создания нового конспекта
        if st.button("➕ Новый конспект"):
            st.session_state.clear()
            st.rerun()

        # Загрузчик файлов для контекста
        uploaded_file = st.file_uploader(
            "Прикрепите файл (.txt, .md)",
            type=['txt', 'md']
        )
        
        # Сохраняем содержимое файла для использования в графе
        file_content_on_start = ""
        if uploaded_file:
            file_content_on_start = uploaded_file.read().decode("utf-8")
            st.info("Файл будет использован при следующем запуске.")

        st.divider()
        
        # История чатов
        st.header("История чатов")
        chats = get_chats()
        if not chats:
            st.caption("История пока пуста.")
        else:
            # Отображаем каждый чат как кнопку
            for chat in chats:
                if st.button(
                    f"📄 {chat['title']}",
                    key=f"chat_{chat['id']}",
                    use_container_width=True
                ):
                    load_chat(chat['id'])
    
    return file_content_on_start


def render_messages():
    """
    Отображает все сообщения текущего чата.
    
    Показывает сообщения пользователя и системы в хронологическом порядке
    с соответствующими аватарами.
    """
    for msg in st.session_state.messages:
        # Выбираем аватар в зависимости от автора
        avatar = "🧑‍💻" if msg['author'] == 'user' else "🤖"
        
        with st.chat_message(msg['author'], avatar=avatar):
            st.markdown(msg['content'])

